...
public class EasyTouch : MonoBehaviour {
	...
	public static bool IsRectUnderTouch( Rect rect, bool guiRect=false){
		...
		*/
		...
		//mod by kin
		for (int i=0;i<10;i++){  
			if ( EasyTouch.instance.fingers[i]!=null && (EasyTouch.instance.fingers[i].position.x > Screen.width/2)){//这里添加了后半部分的识别，//因为我的按钮在左边，不识别右边的手指  
				if (guiRect){  
					rect = new Rect( rect.x,Screen.height-rect.y-rect.height,rect.width,rect.height);     
				}  
				//Debug.LogError("IsRectUnderButton " + i + "   "+ EasyTouch.instance.fingers[i].position);  
				find = rect.Contains(  EasyTouch.instance.fingers[i].position);  
				break;  
			}  
		}
